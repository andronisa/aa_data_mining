...
if __name__ == '__main__':
    """
    Writes the API key to alchemy_key.txt file. It will create the file if it doesn't exist.
    ...
    if len(sys.argv) == 2 and sys.argv[1]:
        if len(sys.argv[1]) == 40:
            ...
            f = open(os.path.join(CONFIG_PATH, 'alchemy_key.txt'), 'w')
            f.write(sys.argv[1])
            f.close()
            print('Key: ' + sys.argv[1] + ' was written to alchemy_key.txt')
            print(
...
class AlchemyAPI:
    ...
    def __init__(self):
        ...
        It loads the API key from alchemy_key.txt and configures the endpoints.
        """

        import sys
        try:
            # Open the key file and read the key
            f = open(os.path.join(CONFIG_PATH, 'alchemy_key.txt'), "r")
            key = f.read().strip()

            if key == '':
                # The key file should't be blank
                print(
                    'The alchemy_key.txt file appears to be blank, please run: python api.py YOUR_KEY_HERE')
                print(
                ...
                print(
                    'It appears that the key in alchemy_key.txt is invalid. Please make sure the file only includes the API key, and it is the correct one.')
                sys.exit(0)
            else:
                # setup the key
                self.apikey = key
            ...
            print(
                'API Key not found! Please run: python api.py YOUR_KEY_HERE')
            print(
                'If you do not have an API Key from AlchemyAPI, please register for one at: http://www.alchemyapi.com/api/register.html')

            # create a blank key file
            open(os.path.join(CONFIG_PATH, 'alchemy_key.txt'), 'a').close()
            sys.exit(0)
